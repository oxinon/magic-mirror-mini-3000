...
WIDGET_ORDER = ["clock", "weather", "air_quality", "warnings", "calendar", "crypto", "stocks", "ews", "defcon"]
WIDGET_LABELS = {
    "clock": "Clock", "weather": "Weather", "air_quality": "Air Quality",
    "warnings": "Warnings", "calendar": "Calendar", "crypto": "Crypto",
    "stocks": "Stocks", "ews": "Apocalypse EWS", "defcon": "DEFCON",
}
...
def draw_boot_screen(d, msg, sub=""):
    d.clear(disp.BLACK)
    d.text_centered("Magic Mirror Mini", disp.WIDTH // 2, 55, disp.ACCENT, "medium")
    d.text_centered(msg, disp.WIDTH // 2, 92, disp.FG, "small")
    if sub:
        d.text_centered(sub, disp.WIDTH // 2, 112, disp.FG_DIM, "small")
    d.show()
...
async def display_task(d, cfg):
    ...
    vor/zurueck. Die Uhr wird weiterhin jede Sekunde neu gezeichnet,
    unabhaengig vom Modus. Welche Widgets ueberhaupt in der Rotation
    auftauchen, wird bei jedem Zeichnen frisch aus der Konfiguration
    gelesen (Web-UI: Widgets einzeln ein-/ausschaltbar)."""
    idx = 0
    manual = False
